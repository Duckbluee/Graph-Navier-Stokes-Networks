...
from GNN_pe import GNN
from GNN_early import GNNEarly
from data import get_dataset, set_train_val_test_split
from heterophilic import get_fixed_splits
from graph_rewiring import apply_beltrami
from torch import nn
from Texas_params import best_params_dict, hetero_params, shared_gnsn_params, shared_grand_params
...
def test(model, data, pos_encoding=None, opt=None):  # opt required for runtime polymorphism
    ...
    logits = model(feat, pos_encoding)
    accs = []
...
def main(cmd_opt):
    ...
    if opt['function'] == 'gnsn':
        opt = shared_gnsn_params(opt)
    ...
    for rep in range(opt['num_splits']):
        ...
        if opt['geom_gcn_splits'] and opt['dataset'] not in ["CoauthorCS", "Computers", "Photo"]:
            ...
            dataset.data = data
        elif opt['dataset'] in ["CoauthorCS", "Computers", "Photo"]:
            dataset.data = set_train_val_test_split(np.random.randint(0, 1000), dataset.data, num_development=5000)
        ...
        if opt['function'] in ['laplacian', 'gnsn']:
        ...
        for epoch in range(1, opt['epoch']):
            ...
            if tmp_test_acc > test_acc and tmp_val_acc >= val_acc:
            ...
            print(f"Epoch: {epoch}, Runtime: {time.time() - start_time:.4f}, Loss: {loss:.3f}, "
                ...
                f"Train: {train_acc:.4f}, Val: {val_acc:.4f}, Ave: {test_acc:.4f}")
            ...
            if opt['patience'] is not None:
                if patience_count >= opt['patience']:
                    break
        print(
            f"best val accuracy {val_acc:.3f} with best test accuracy {best_acc:.3f} at epoch {best_epoch}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_best_params', default=True, help="flag to take the best params")
    ...
    # data args
    parser.add_argument('--dataset', type=str, default='texas', help='Cora, Citeseer, Pubmed, Computers, Photo, CoauthorCS, ogbn-arxiv')
    ...
    parser.add_argument('--function', type=str, default='gnsn', help='laplacian, transformer, greed, GAT')
    ...
    parser.add_argument('--hg', type=float, default=0.5)
