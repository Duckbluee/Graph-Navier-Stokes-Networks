...
class ConstantODEblock(ODEblock):
  def __init__(self, odefunc, regularization_fns, opt, data, device, node_t, t=torch.tensor([0, 1])):
    super(ConstantODEblock, self).__init__(odefunc, regularization_fns, opt, data, device, t, node_t)
    ...
    if opt['data_norm'] == 'rw':
      ...
      edge_index, edge_weight = gcn_norm_fill_val(data.edge_index, edge_weight=data.edge_attr,
                                                  ...
                                                  num_nodes=data.num_nodes,
                                                  dtype=data.x.dtype)
    self.odefunc.node_t = Parameter(node_t).to(device)
    self.odefunc.edge_index = edge_index.to(torch.int64).to(device)
