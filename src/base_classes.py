...
class ODEblock(nn.Module):
  def __init__(self, odefunc, regularization_fns, opt, data, device, t, node_t):
...
class BaseGNN(MessagePassing):
  def __init__(self, opt, dataset, node_t, device=torch.device('cpu')):
    ...
    self.linear = nn.Linear(dataset.data.x.shape[1], dataset.data.x.shape[1]).to(self.device)
    self.linear1 = nn.Linear(dataset.data.x.shape[1], opt['hidden_dim']).to(self.device)
    self.timelinear = nn.Linear(dataset.data.x.shape[0], dataset.data.x.shape[0]).to(self.device)
    ...
    if opt['beltrami']:
      ...
      self.m1 = nn.Linear(2*self.num_features, opt['hidden_dim'])
